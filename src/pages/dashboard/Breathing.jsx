...
const Breathing = () => {
  ...
  return (
    <div className="p-6 space-y-6 breathing">
      <h1 className="text-3xl font-bold text-center mb-6 text-blue-600">Breathing Exercises</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {exercises.map((exercise) => (
          ...
          >
            <h2 className="text-lg font-semibold text-blue-600">{`Level ${exercise.level}: ${exercise.name}`}</h2>
            <p className="text-sm  mt-2 text-blue-600">{exercise.description}</p>
           <div className='flex items-center justify-center mt-1 text-xl'> <GoVideo className='text-blue-600  '/></div>
          </div>
      ...
      {activeExercise && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg shadow-lg text-center space-y-4 max-w-lg w-full h-4/5 overflow-y-auto">
            <h2 className="text-xl font-semibold text-blue-600">{`Level ${activeExercise.level}: ${activeExercise.name}`}</h2>
            <p className=" text-blue-600">{activeExercise.description}</p>
